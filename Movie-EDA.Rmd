...
movie_data$release_date <- as.Date(movie_data$release_date, format = "%m/%d/%y")
head(movie_data$release_date)
```
...
library(purrr)
test <- separated %>% select(Country1:Country12) %>% map_df(~country_name(.)) %>% bind_cols(id = separated$id)
countries <- test %>% pivot_longer(Country1:Country12, names_to = "CountryNumber", values_to = "Country", values_drop_na = TRUE)
countries %>% 
  group_by(Country) %>%
  summarize(count = n()) %>% 
  arrange(desc(count))
...
library(forcats)
another_test <- countries %>% 
  mutate(Country = factor(Country),
         Country = fct_lump_min(Country, min=100, other_level = "Other"))
head(another_test)

library(fastDummies)
dummy_test <- another_test %>% dummy_cols(select_columns = "Country")
head(dummy)
backwards <- another_test %>% pivot_wider(id_cols = id, names_from = CountryNumber, values_from=Country)


third_test <- backwards %>% unite("AllCountries", Country1:Country12, sep = ", ")

clean <- third_test %>% mutate(AllCountries = list(str_remove_all(AllCountries, pattern = ", NA")))
many_test <- sapply(clean$AllCountries, function(x){unlist(str_extract_all(x, '([A-Za-z])\\w+(,)'))})
                    head(many_test)
head(clean)
library(qdapTools)

qdapTools::mtabulate(as.list(clean$AllCountries))
test_list <- as.list(clean$AllCountries)
head(test_list)
head(movie_data$genre_list)
test_list[[1]]
movie_data$genre_list[[1]]
```
