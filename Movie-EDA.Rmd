...
# Changing NAs to "zz"
movie_data[is.na(movie_data$spoken_languages),]$spoken_languages <- "zz"
```
...
movie_data <- movie_data %>% mutate(SpNone = ifelse(sapply(movie_data$spoken_languages, function(x){any(str_detect(x, 'zz'))}), 1, 0))
```
